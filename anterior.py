...
class PoseAlertLegsAndArms(nn.Module):
    ...
    def __init__(self,
                 ...
                 margem_pct_altura_bbox: float = 0.12,  # 8% da ALTURA da bbox
    ...
    def forward(self, keypoints, image_width, image_height, num_pessoas_tensor, yolo_bboxes):
        ...
        olho_esq_px = olho_esq
        olho_dir_px = olho_dir
        desloc_esq = 5.0 * torch.abs(orelha_esq[1] - olho_esq_px[1])
        desloc_dir = 5.0 * torch.abs(orelha_dir[1] - olho_dir_px[1])
        ...
        # ==========================================================
        # Esperado: yolo_bboxes.shape == (N,4) em pixels, mesma ordem dos keypoints.
        # Usamos a 1ª pessoa (linha 0). Se for inválida, caímos no fallback via keypoints-bbox.
        if yolo_bboxes is None or yolo_bboxes.numel() == 0:
            x1 = y1 = x2 = y2 = None
        else:
            bb = yolo_bboxes[0]  # primeira pessoa
        ...
        if x1 is not None:
            ...
            # Fallback: usa bbox derivada dos keypoints visíveis 
            xs = key_px[:, 0]
            ys = key_px[:, 1]
            mask = (valid > 0).bool()
